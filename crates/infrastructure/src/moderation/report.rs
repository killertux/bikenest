...
impl ReportRow {
    fn into_report(self) -> Result<Report, ModerationError> {
        let target_type = ReportTargetType::from_code(&self.target_type)
...
#[derive(sqlx::FromRow)]
        ...
        let row = sqlx::query_as::<_, IdRow>(r#"
            ...
            RETURNING id
            "#).bind(r.reporter_id.0).bind(r.target_type.as_code()).bind(r.target_id).bind(&r.reason).bind(r.description.as_str())
    ...
    async fn get(&self, id: i64) -> Result<Option<Report>, ModerationError> {
        let row = sqlx::query_as::<_, ReportRow>(r#"
            SELECT id, reporter_id, target_type, target_id, reason, description, state,
                   claimed_by, resolved_by, resolution_note, created_at, updated_at
            FROM report
            WHERE id = $1
    ...
    async fn resolve(
        ...
        let res = sqlx::query(r#"
            ...
            "#).bind(id).bind(moderator.0).bind(outcome.as_code()).bind(note)
        .execute(self.db.pool())
        .await
        .map_err(map_err)?;
        if res.rows_affected() != 1 {
