...
impl AnonymizationRepository for SqlxAnonymizationRepository {
    ...
    async fn anonymize(
        ...
        sqlx::query(r#"
            ...
            WHERE id = $1
            "#).bind(user_id.0).bind(scrub_email).bind(now)
        .execute(&mut *tx)
        .await
        .map_err(map_err)?;

        let identities = sqlx::query("DELETE FROM authentication_identities WHERE user_id = $1").bind(user_id.0)
            .execute(&mut *tx).await.map_err(map_err)?.rows_affected();
        let roles = sqlx::query("DELETE FROM user_roles WHERE user_id = $1").bind(user_id.0)
            .execute(&mut *tx).await.map_err(map_err)?.rows_affected();
        let sessions = sqlx::query("DELETE FROM sessions WHERE user_id = $1").bind(user_id.0)
            .execute(&mut *tx).await.map_err(map_err)?.rows_affected();
        let email_verification_tokens = sqlx::query("DELETE FROM email_verification_tokens WHERE user_id = $1").bind(user_id.0)
        .execute(&mut *tx).await.map_err(map_err)?.rows_affected();
        let password_reset_tokens = sqlx::query("DELETE FROM password_reset_tokens WHERE user_id = $1").bind(user_id.0)
        .execute(&mut *tx).await.map_err(map_err)?.rows_affected();
        let favorites = sqlx::query("DELETE FROM favorite WHERE user_id = $1").bind(user_id.0)
            .execute(&mut *tx).await.map_err(map_err)?.rows_affected();
        // Parked-here is personal activity → deleted.
        let parked_here = sqlx::query("DELETE FROM verification WHERE user_id = $1 AND kind = 'parked_here'").bind(user_id.0)
        .execute(&mut *tx).await.map_err(map_err)?.rows_affected();
        let exports = sqlx::query("DELETE FROM personal_data_export WHERE user_id = $1").bind(user_id.0)
            .execute(&mut *tx).await.map_err(map_err)?.rows_affected();
        let consent_records = sqlx::query("DELETE FROM consent_record WHERE user_id = $1").bind(user_id.0)
            .execute(&mut *tx).await.map_err(map_err)?.rows_affected();

        // 2) Community content is retained but unattributed.
        let reviews_anonymized = sqlx::query("UPDATE review SET author_id = NULL WHERE author_id = $1").bind(user_id.0)
        .execute(&mut *tx).await.map_err(map_err)?.rows_affected();
        let verifications_anonymized = sqlx::query("UPDATE verification SET user_id = NULL WHERE user_id = $1 AND kind <> 'parked_here'").bind(user_id.0)
        .execute(&mut *tx).await.map_err(map_err)?.rows_affected();
        ...
        .execute(&mut *tx).await.map_err(map_err)?.rows_affected();
        let reports_anonymized = sqlx::query(r#"
            UPDATE report SET reporter_id = NULL, claimed_by = NULL, resolved_by = NULL
            WHERE reporter_id = $1 OR claimed_by = $1 OR resolved_by = $1
            "#).bind(user_id.0)
        .execute(&mut *tx).await.map_err(map_err)?.rows_affected();
        let locations_anonymized = sqlx::query("UPDATE parking_location SET creator_id = NULL WHERE creator_id = $1").bind(user_id.0)
        .execute(&mut *tx).await.map_err(map_err)?.rows_affected();
        let revisions_anonymized = sqlx::query("UPDATE parking_revision SET editor_id = NULL WHERE editor_id = $1").bind(user_id.0)
        .execute(&mut *tx).await.map_err(map_err)?.rows_affected();
        let parking_photos_anonymized = sqlx::query("UPDATE parking_photo SET uploader_id = NULL WHERE uploader_id = $1").bind(user_id.0)
        .execute(&mut *tx).await.map_err(map_err)?.rows_affected();
        let review_photos_anonymized = sqlx::query("UPDATE review_photo SET uploader_id = NULL WHERE uploader_id = $1").bind(user_id.0)
        .execute(&mut *tx).await.map_err(map_err)?.rows_affected();
        let audit_events_anonymized = sqlx::query("UPDATE audit_events SET actor_user_id = NULL WHERE actor_user_id = $1").bind(user_id.0)
        .execute(&mut *tx).await.map_err(map_err)?.rows_affected();
