...
impl AnonymizationRepository for SqlxAnonymizationRepository {
    ...
    async fn anonymize(
        ...
        let identities = sqlx::query!("DELETE FROM authentication_identities WHERE user_id = $1", user_id.0)
            .execute(&mut *tx).await.map_err(map_err)?.rows_affected();
        let roles = sqlx::query!("DELETE FROM user_roles WHERE user_id = $1", user_id.0)
            .execute(&mut *tx).await.map_err(map_err)?.rows_affected();
        let sessions = sqlx::query!("DELETE FROM sessions WHERE user_id = $1", user_id.0)
            .execute(&mut *tx).await.map_err(map_err)?.rows_affected();
        ...
        .execute(&mut *tx).await.map_err(map_err)?.rows_affected();
        let favorites = sqlx::query!("DELETE FROM favorite WHERE user_id = $1", user_id.0)
            .execute(&mut *tx).await.map_err(map_err)?.rows_affected();
        // Parked-here is personal activity → deleted.
        let parked_here = sqlx::query!(
            "DELETE FROM verification WHERE user_id = $1 AND kind = 'parked_here'",
            user_id.0,
        )
        .execute(&mut *tx).await.map_err(map_err)?.rows_affected();
        let exports = sqlx::query!("DELETE FROM personal_data_export WHERE user_id = $1", user_id.0)
            .execute(&mut *tx).await.map_err(map_err)?.rows_affected();
        let consent_records = sqlx::query!("DELETE FROM consent_record WHERE user_id = $1", user_id.0)
            .execute(&mut *tx).await.map_err(map_err)?.rows_affected();

        // 2) Community content is retained but unattributed.
        let reviews_anonymized = sqlx::query!(
            "UPDATE review SET author_id = NULL WHERE author_id = $1",
            user_id.0,
        )
        .execute(&mut *tx).await.map_err(map_err)?.rows_affected();
        ...
        .execute(&mut *tx).await.map_err(map_err)?.rows_affected();
        let proposals_anonymized = sqlx::query!(
            "UPDATE parking_proposal SET proposer_id = NULL, resolved_by = NULL WHERE proposer_id = $1 OR resolved_by = $1",
            user_id.0,
        )
        .execute(&mut *tx).await.map_err(map_err)?.rows_affected();
        ...
        .execute(&mut *tx).await.map_err(map_err)?.rows_affected();
        let locations_anonymized = sqlx::query!(
            "UPDATE parking_location SET creator_id = NULL WHERE creator_id = $1",
            user_id.0,
        )
        .execute(&mut *tx).await.map_err(map_err)?.rows_affected();
        let revisions_anonymized = sqlx::query!(
            "UPDATE parking_revision SET editor_id = NULL WHERE editor_id = $1",
            user_id.0,
        )
        .execute(&mut *tx).await.map_err(map_err)?.rows_affected();
        let parking_photos_anonymized = sqlx::query!(
            "UPDATE parking_photo SET uploader_id = NULL WHERE uploader_id = $1",
            user_id.0,
        )
        .execute(&mut *tx).await.map_err(map_err)?.rows_affected();
        let review_photos_anonymized = sqlx::query!(
            "UPDATE review_photo SET uploader_id = NULL WHERE uploader_id = $1",
            user_id.0,
        )
        .execute(&mut *tx).await.map_err(map_err)?.rows_affected();
        let audit_events_anonymized = sqlx::query!(
            "UPDATE audit_events SET actor_user_id = NULL WHERE actor_user_id = $1",
            user_id.0,
        )
        .execute(&mut *tx).await.map_err(map_err)?.rows_affected();
        let privacy_requests_anonymized = sqlx::query!(
            "UPDATE privacy_request SET user_id = NULL, fulfilled_by = NULL WHERE user_id = $1 OR fulfilled_by = $1",
            user_id.0,
        )
        .execute(&mut *tx).await.map_err(map_err)?.rows_affected();
