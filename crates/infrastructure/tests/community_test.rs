...
async fn create_writes_location_revision_and_reads_back(tx: &mut bikenest_test_support::TestTx) {
    ...
    let id = repo.create(&new_location(), user, chrono::Utc::now()).await.unwrap();
    assert!(id > 0);
...
async fn optimistic_edit_wins_only_on_expected_version(tx: &mut bikenest_test_support::TestTx) {
    ...
    let new_version = repo.apply_edit(id, 1, &edit, user, chrono::Utc::now()).await.unwrap();
    assert_eq!(new_version, 2);
    let current = repo.get_for_edit(id).await.unwrap().unwrap();
    assert_eq!(current.version(), 2);

    // Stale version → conflict, no change.
    let err = repo.apply_edit(id, 1, &edit, user, chrono::Utc::now()).await.unwrap_err();
...
async fn review_upsert_recomputes_rating_and_appends_history(
    ...
    reviews.upsert_review(id, user, StarRating::new(4).unwrap(), &body).await.unwrap();
    reviews.upsert_review(id, user2, StarRating::new(5).unwrap(), &body).await.unwrap();
    ...
    assert!(existed);
    reviews.upsert_review(id, user, StarRating::new(2).unwrap(), &body2).await.unwrap();
    let active = reviews.list_active(id).await.unwrap();
    assert_eq!(active.len(), 2);
...
async fn verification_still_exists_sets_last_verified_at(tx: &mut bikenest_test_support::TestTx) {
    ...
    ver.record(&NewVerification::Existence {
        location_id: id,
        user_id: user,
    ...
    let verified: (bool,) = sqlx::query_as(
        "SELECT last_verified_at IS NOT NULL FROM parking_location WHERE id = $1",
    )
    .bind(id)
    ...
    ver.record(&NewVerification::Attribute {
        location_id: id,
        user_id: user,
        code: "name".to_string(),
    ...
    ver.record(&NewVerification::ParkedHere {
        location_id: id,
        user_id: user,
    }, chrono::Utc::now())
    .await
