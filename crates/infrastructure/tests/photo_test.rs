...
fn jpeg_with_exif_orientation(base: &[u8], orientation: u8) -> Vec<u8> {
    assert!(base.len() >= 4 && base[0] == 0xFF && base[1] == 0xD8, "SOI expected");
...
async fn repo_approve_sets_position_and_reviewer(tx: &mut bikenest_test_support::TestTx) {
    ...
    repo.mark_processed(
        ...
        "uploads/1/full.jpg",
        "uploads/1/thumb.jpg",
        PhotoDimensions { width: 10, height: 10 },
        chrono::Utc::now(),
    )
    .await
    .unwrap();
...
async fn reader_returns_thumbnail_key_for_processed_photo(tx: &mut bikenest_test_support::TestTx) {
    ...
    let p = photos.iter().find(|p| p.alt.as_deref() == Some("An alt text")).expect("photo");
