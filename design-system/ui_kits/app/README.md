...
3. For full-screen composition, choose the shell (public / authenticated / moderator) from the kit and reuse these components inside it.
...
- **Shell choice:** for full screens, pick the public / authenticated / moderator shell and drop kit components inside.
- **Token discipline:** bind `../../../colors_and_type.css` first; never introduce raw colors in page styles.
- **Named building blocks:** the kit shell (`index.html`), `Sidebar`-style moderator tabs (`components/navigation.html`), `Composer`-style form stacks (`components/forms.html`), and `PreviewCard`-style spot cards (`components/cards.html`) map one-to-one onto the token-bound patterns.
...
Component shapes follow the source token system (cards, radio cards, queues, scrim badges, reports tables, auth fields, the landing header — see `context/provenance.md`). Imagery comes from `assets/imagery/` (Pexels-licensed). Typography and colors are the source token system: Avenir Next display, system body, SF Mono data, OKLch hue-240 neutrals with the hue-170 green-teal accent.
