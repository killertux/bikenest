...
- Handoff context: `context/source-context.md`.
...
- `assets/imagery/` contains the seven optimized photographs shipped with the source project (also preserved unoptimized under `images/`). They are **Pexels** photos, referenced by filename in the source screens. Pexels license permits use without attribution; filenames retain photographer attribution:
...
- No logos, wordmarks, app icons, or font files existed in the source evidence; the wordmark is set in the display font stack (Avenir Next fallback). `build/` therefore contains icons only and `fonts/` is intentionally absent.

## Preserved source code

- All 33 generated screens (`p*`, `a*`, `c*`, `d*`, `m*`, `e*`), the three generator scripts, and five render-check snapshots remain untouched at the workspace root. They are source evidence and the highest-signal component reference; the design-system outputs (`preview/`, `ui_kits/app/`) are new files built on top of their token system, not replacements.
- `render-checks/*.png` are verification renders of the source screens (1440–2880px wide) and are reused as thumbnails in `preview/applied-ui.html`.
