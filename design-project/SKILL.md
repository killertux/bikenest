...
- `build/icons/` — 25 Lucide runtime icons used by the source screens (ISC license).
- `preview/` — focused review cards (colors, typography, spacing, components, brand assets, applied UI).
...
- The 33 preserved source screens at the project root (`p*`, `a*`, `c*`, `d*`, `m*`, `e*`) — the highest-signal reference for component shapes and copy tone.
...
- Compare component shapes against the preserved source screens (`p3-parking-details.html` for cards/forms, `m2-photos.html` for queues, `a1-register.html` for auth forms), and against the applied kit in `ui_kits/app/`.
