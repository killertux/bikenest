...
| Object-storage provider (**S3-compatible** / MinIO in dev) | photo binaries | derivative bytes under opaque keys (§77: no user metadata in keys) | ⚠ review | processor | HTTPS (direct S3 presigned URLs; the app is not a proxy) | until rejected/anonymized | `ObjectStorage::delete` |
