...
|---|---|---|---|---|---|
| 1 | `FakeGeocoder` (deterministic Curitiba landmarks + fallback) | `crates/infrastructure/src/geocoding.rs`; wired in `http.rs` | `RealGeocoder` behind the `Geocoder` port — self-hosted **Photon** (OSM data) recommended, or a hosted OSM-derived provider | product/ops: provider choice + hosting + API key + ToS/usage limits | Ledger #2. §83: public Nominatim is not production infra. §77: query string only, no account identity |
...
| 5 | `InMemoryRateLimiter` (per-process, per-service instance) | `crates/infrastructure/src/auth/rate_limit.rs`; constructed **4×** in `http.rs` (auth, contributions, photos, moderation) | `RedisRateLimiter` behind the `RateLimiter` port, one **shared** instance wired into all services | ops: Redis service + `REDIS_URL`; or an explicit decision to stay single-instance | Ledger #6. Today limits silently reset across instances/restarts and don't aggregate |
